...
func main() {
	...
	go func() {
		// Simulate human decision - REJECTED
		select {
		case humanStepID := <-humanStepIDCh:
			time.Sleep(time.Second)
			decision := floxy.HumanDecisionRejected
			decidedBy := "manager@company.com"
			comment := "Document does not meet quality standards and needs revision"
...
attempts:
	for i := 0; i < maxAttempts; i++ {
		...
		for _, step := range steps {
			...
			if step.StepName == "send-rejection-notification" && step.Status == floxy.StepStatusCompleted {
				log.Printf("Rejection notification sent successfully!")
