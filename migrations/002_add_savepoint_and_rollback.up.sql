...
ALTER TABLE workflows.workflow_steps ADD CONSTRAINT workflow_steps_step_type_check 
    CHECK (step_type IN ('task', 'parallel', 'condition', 'fork', 'join', 'save_point'));
...
-- Update comments
COMMENT ON COLUMN workflows.workflow_steps.step_type IS 'task | parallel | condition | fork | join | save_point';
COMMENT ON COLUMN workflows.workflow_steps.status IS 'pending | running | completed | failed | skipped | rolled_back';
