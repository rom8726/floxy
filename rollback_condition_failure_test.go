...
func TestRollbackConditionFailureInParallelBranches(t *testing.T) {
	...
	workflowDef, err := NewBuilder("rollback_failure_parallel", 1).
	...
	instanceID, err := engine.Start(ctx, "rollback_failure_parallel-v1", input)
	require.NoError(t, err)
	...
	// in parallel branches when one branch fails
...
func TestRollbackConditionFailureInParallelBranchesWithFailure(t *testing.T) {
	container, pool := setupTestDatabase(t)
	t.Cleanup(func() {
		pool.Close()
		_ = container.Terminate(context.Background())
	...
	// Create workflow with condition in parallel branch where one branch fails
	workflowDef, err := NewBuilder("rollback_failure_parallel2", 1).
		Step("start", "simple-test", WithStepMaxRetries(1)).
		Fork("parallel_branch", func(branch1 *Builder) {
			branch1.Step("branch1_step1", "simple-test", WithStepMaxRetries(1)).
				Condition("branch1_condition", "{{ gt .count 5 }}", func(elseBranch *Builder) {
	...
	// Start with count = 2 (branch1: false -> else, branch2: true -> next, but else will fail)
	input := json.RawMessage(`{"count": 2}`)
	instanceID, err := engine.Start(ctx, "rollback_failure_parallel2-v1", input)
	require.NoError(t, err)
