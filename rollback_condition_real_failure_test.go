...
func TestRollbackConditionRealFailureInParallelBranches(t *testing.T) {
	if testing.Short() {
	...
	workflowDef, err := NewBuilder("rollback_real_failure", 1).
...
func TestRollbackConditionRealFailureInParallelBranchesWithFailure(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping test in short mode.")
	}
	...
	// Create workflow with condition in parallel branch where one branch fails
	workflowDef, err := NewBuilder("rollback_real_failure2", 1).
		Step("start", "simple-test", WithStepMaxRetries(1)).
		Fork("parallel_branch", func(branch1 *Builder) {
			branch1.Step("branch1_step1", "simple-test", WithStepMaxRetries(1)).
				Condition("branch1_condition", "{{ gt .count 5 }}", func(elseBranch *Builder) {
